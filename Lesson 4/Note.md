...
- 優點: 很好理解，而且可以簡寫 (參考 Solution 1b)。
- 缺點: 每次實例組件時，都會創建一個新的函數，會影響效能。

### 方法二： 定義操作當作事件處理函數

- 在定義操作時，使用一般函數，則在使用時，需要再使用箭頭函數包裹一層。
...
- 優點: 這個方法的優點是可以保持事件處理器的傳統語法（即非箭頭函數的寫法），並且可以在不同的方法間共享 this。儘管它可能會影響效能，但在某些情況下，這種方法提供了更清晰的代碼結構。
...
- 組件函數中的`this`指向：在 React 組件中，函數中的`this`指向是`undefined`。因此，如果要使用`this`
  ...
  組件類別中的`this`)。
- 為了把事件處理函數與JSX分開，使之能更好維護，有三種方法：
  - 方法一：在 `constructor()` 中定義箭頭函數，或簡寫。
  - 方法二： 定義操作當作事件處理函數
  - 跳過 (~~方法三：在 `render()` 中定義箭頭函數~~)
  - 方法四：在`constructor()`中使用`bind()`方法
- 如果是普通函數操作，則必須在使用時外部用額外的箭頭函數包裹一層或在`constructor()`中使用`bind()`方法。
- 不要在`render()`方法中定義箭頭函數，因為每次渲染都會產生新的函數，會嚴重影響效能。
- 最後，建議使用方法一或方法二。方法一簡寫比較常用，但是方法二效能最好。